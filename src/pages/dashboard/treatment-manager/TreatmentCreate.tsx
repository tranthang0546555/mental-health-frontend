...
export default function TreatmentCreate() {
    ...
    const onSubmit = async (data: Inputs) => {
        ...
        await useApi.post(url, formData).then(() => {
            toast.success("Đã thêm mới");
            reset();
        });
    };

    const handleFileChange = (file: File, type?: 'video' | 'audio') => {
        if (!type) {
...
type FileUploadProps = {
    handleChange: (file: File, type?: 'video' | 'audio') => void
}
const FileUpload = (props: FileUploadProps) => {
    const onChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (videoValid.includes(file.type)) _type = 'video';
        if (audioValid.includes(file.type)) _type = 'audio';
        props.handleChange(file, _type);
    };
